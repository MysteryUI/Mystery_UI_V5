	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
			bar.ApplyStyle=function()
				...
				if icon1 then
					icon1:SetSize(16, 16)
					icon1:SetPoint("RIGHT", frame, "LEFT", -6, 5)
				end

				if icon2 then
					icon2:SetSize(16, 16)
					icon2:SetPoint("LEFT", frame, "RIGHT", 6, 5)
				end
				...
				if not name.styled then
					name:ClearAllPoints()
					name:SetPoint("LEFT", frame, "LEFT", 4, 13)
					name:SetWidth(165)
					name:SetHeight(8)
				...
				if not timer.styled then	
					...
					timer:SetPoint("RIGHT", frame, "RIGHT", -4, 13)
					timer:SetFont("Fonts\\ARIALN.TTF", 12, "THINOUTLINE")
...
local UploadDBM = function() 
   ...
   DBM_SavedOptions.RangeFrameX = 245 
