...
function addon:ADDON_LOADED(event, addon)
	if addon == "ControlTimers" then
		...
		self.db = self.CopyDefaults(defaults, ControlTimersDB)
...
function addon:PLAYER_LOGIN(event)
	if IsAddOnLoaded("AuraTimers")  then -- check for old addon
		self.AT_loaded = true
		DEFAULT_CHAT_FRAME:AddMessage("|c00FF9900[PT]: |c00FF0000Warning!|r You must remove any previous versions of AuraTimers before installing ControlTimers!")
		return
	end
	if IsAddOnLoaded("PortraitTimers")  then -- check for old addon
		self.AT_loaded = true
		DEFAULT_CHAT_FRAME:AddMessage("|c00FF9900[PT]: |c00FF0000Warning!|r You must remove any previous versions of PortraitTimers before installing ControlTimers!")
		return
	end
...
function addon.ListAuras(unit)
	...
	repeat
		--print(i)
		name, _, texture, _, _, _, eTime = UnitAura(unit, i)							
		if (name ~= nil) then
	...
	i = 1
	repeat
		name, _, texture, _, _, _, eTime = UnitDebuff(unit, i)							
		if (name ~= nil) then
			table.insert(auras, {	["name"] = name, 
									["texture"] = texture, 
									["eTime"] = eTime - GetTime()})
...
function addon.CheckAura(unit)
	...
	for idx, spellID in ipairs(addon.spellDB) do
		...
		if (spell ~= nil) then
			--print("checking aura " .. spell .. " on " .. unit)
			for i = 1, #auCheck do						
		...
		for i, au in ipairs(auras) do
			...
			if (#frame.auras > 0) then
				for j, fau in  ipairs(frame.auras) do
					if (au.spell == fau.spell) then flag = true end
				end
			end
			if not flag then
...
function addon.UpdateTimer(self, elapsed)
	-- emergency timer
	if (self.tick == nil) then self.tick = 0.5 end
	self.check = false
